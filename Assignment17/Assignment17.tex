...
\hline
    ...
    &\\
    &$\qquad\qquad\qquad \vec{A}=\myvec{a&b\\c&d}$\\
    &\\
    & $\qquad\qquad\qquad \det{\vec{A}} = \mydet{a&b\\c&d}$\\ &\\
    & $\qquad\qquad\qquad\boxed{\det{\vec{A}} = ad-bc}$\\
    &\\
    & $\qquad\qquad\qquad\boxed{ tr(\vec{A})=\brak{a+d}}$\\
    &\\
    & Now, \\
    & $\qquad\qquad\qquad(\vec{A}+\vec{I}) = \myvec{a+1&b\\c&d+1}$\\
    & {\begin{align}\label{eq1}\det(\vec{A}+\vec{I})=\mydet{a+1&b\\c&d+1}=\brak{a+1}\brak{d+1}-\brak{bc}\end{align}}\\
    \hline
    
    & {\begin{align}\label{eq2}\boxed{1+\det{\vec{A}}= 1+\brak{ad-bc}}\end{align}}\\
    
    
    & Since, $\det\brak{\vec{A}+\vec{I}}=\det\brak{\vec{A}}$\\
    &\\
    & Equating \eqref{eq1} and \eqref{eq2};\\
    &\\
    &$\qquad\qquad\qquad\brak{a+1}\brak{d+1}-\brak{bc}=1+\brak{ad-bc}$\\
    &\\
    &$\qquad\qquad\qquad \implies\boxed {\brak{a+d}=0=tr(\vec{A})}$\\
    &\\
    \hline
	\multirow{3}{*}{Option 1 } 
	& \\
	& Let,\\
	& $\qquad\qquad\qquad\vec{A}=\myvec{0&0\\0&-1}$\\
	&\\
	& $\qquad\qquad\qquad\det{\vec{A}} = \mydet{0&0\\0&-1} = 0$\\
	&\\
	& $\qquad\qquad\qquad\brak{\vec{A}+\vec{I}}=\myvec{1&0\\-1&1}$\\
	&\\
	& $\qquad\qquad\qquad\det\brak{\vec{A}+\vec{I}}=\mydet{1&0\\-1&0}=1$\\
	&\\
    ...
    &\\
    & Eigen value of $\vec{A}=\lambda_1,\lambda_2$\\
    &\\
    & Eigen value of $\brak{\vec{A}+\vec{I}} = \lambda_1+1,\lambda_2+1$\\
    &\\
    & Since,$\det\brak{\vec{A}+\vec{I}}=1+\det(\vec{A})$\\
    &\\
    & $\qquad\qquad\implies \brak{\lambda_1+1}\brak{\lambda_2+1}=1+\brak{\lambda_1\lambda_2}$\\
    &\\
    &$\qquad\qquad\implies \lambda_1+\lambda_2 = 0$\\
    &\\
    & trace of any matrix is sum of its eigen values. \\
    &\\
    &$\qquad\qquad\implies tr(\vec{A})=0$\\
    &\\
    \hline
	\multirow{3}{*}{Option 4}&\\
